...
function TC_Dashboard_Solution_Workbench_1()
{
    ...
    //--- Creates External BAQs -----------------------------------------------------------------------------------------------------------------'
        ...
        Log["Message"]("Grid adapter data filled")
    ...
    */
        ...
        // Shared: Checked
        CreateBAQ(baqs2, baqs2, "chkShared")
    ...
    */
        ...
        // Shared: Checked
        CreateBAQ(baqs3, baqs3, "chkAllCompanies,chkShared")
        Delay(2000)
    ...
    */
        Log["Message"]("Step 13")
        NewDashboard(dashb3, dashb3, dashb3, "chkAllCompanies")
...
//--- On EPIC06 Install the exported solution: ----------------------------------------------------------------------------------------------'
  ...
  //-------------------------------------------------------------------------------------------------------------------------------------------' 
    ...
    if (Aliases["Epicor"]["Dashboard"]["dbPanel"]["windowDockingArea1"]["dockableWindow2"]["pnlGeneral"]["windowDockingArea1"]["dockableWindow1"]["pnlGenProps"]["txtCaption"] != "") {
    Login["Checkpoint"] ("Dashboard " + dashb2 + "  retrieved")
    }else{
    Login["Error"] ("Dashboard " + dashb2 + "  wasn't retrieved")
    }

    if(!Aliases["Epicor"]["Dashboard"]["dbPanel"]["windowDockingArea1"]["dockableWindow2"]["pnlGeneral"]["windowDockingArea1"]["dockableWindow1"]["pnlGenProps"]["chkAllCompanies"]["Checked"]) {
    Login["Checkpoint"] ("Dashboard " + dashb2 + "  'All Companies' checkbox is not checked")
    }else{
    Login["Error"] ("Dashboard " + dashb2 + " 'All Companies' checkbox is checked")
    }
    ...
    for (i = 0; i < advElementsGrid["Rows"]["Count"]; i++)
    {
        ...
        for (j = 0; j < dashboardGrid["length"]; j++)
        {
            if (cellDefID["Text"]["OleValue"] == dashboardGrid[j])
            {
                ...
                Log["Error"]("Dashboard " +  dashboardGrid[j] + " not found on grid")
                break
            }
        }
